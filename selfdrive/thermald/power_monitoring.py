...
class PowerMonitoring:
    ...
    def calculate(self, pandaState):
        try:
            ...
            self.car_voltage_mV = ((pandaState.pandaState.voltage * CAR_VOLTAGE_LOW_PASS_K) + (
            ...
            if (pandaState.pandaState.ignitionLine or pandaState.pandaState.ignitionCan):
                ...
                if current_power is not None:
                    ...
                    current_power = (
                                (HARDWARE.get_battery_voltage() / 1000000) * (HARDWARE.get_battery_current() / 1000000))
                elif (pandaState.pandaState.pandaType in [log.PandaState.PandaType.whitePanda,
                                                          log.PandaState.PandaType.greyPanda]) and (
                        pandaState.pandaState.current > 1):
                    ...
                    current_power = (
                                PANDA_OUTPUT_VOLTAGE * panda_current_to_actual_current(pandaState.pandaState.current))
        ...
        with self.integration_lock:
            try:
                if self.last_measurement_time:
                    ...
                    if power_used < 0:
                        raise ValueError(
                            f"Negative power used! Integration time: {integration_time_h} h Current Power: {power_used} uWh")
    ...
    def should_shutdown(self, pandaState, offroad_timestamp, started_seen):
        ...
        should_shutdown |= (
                    (HARDWARE.get_battery_capacity() < BATT_PERC_OFF) and (not HARDWARE.get_battery_charging()) and (
                        (now - offroad_timestamp) > 60))
