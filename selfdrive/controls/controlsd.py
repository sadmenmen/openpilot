...
IGNORE_PROCESSES = set(
    ["rtshield", "uploader", "deleter", "loggerd", "logmessaged", "tombstoned", "logcatd", "proclogd", "clocksd",
     "updated", "timezoned", "manage_athenad"])
...
class Controls:
    def __init__(self, sm=None, pm=None, can_sock=None):
        ...
        if self.sm is None:
            ...
            self.sm = messaging.SubMaster(['deviceState', 'pandaState', 'modelV2', 'liveCalibration',
                                           ...
                                           'liveLocationKalman',
                                           'roadCameraState', 'driverCameraState', 'managerState', 'liveParameters',
                                           'radarState'], ignore_alive=ignore)
    ...
    def state_control(self, CS):
        ...
        # Gas/Brake PID loop
        actuators.gas, actuators.brake = self.LoC.update(self.active, CS, v_acc_sol, long_plan.vTargetFuture, a_acc_sol,
                                                         self.CP)

        # Steering PID loop and lateral MPC
        actuators.steer, actuators.steeringAngleDeg, lac_log = self.LaC.update(self.active, CS, self.CP, self.VM,
                                                                               params, lat_plan)

        # Check for difference between desired angle and angle for angle based control
        angle_control_saturated = self.CP.steerControlType == car.CarParams.SteerControlType.angle and \
                                  abs(
                                      actuators.steeringAngleDeg - CS.steeringAngleDeg) > STEER_ANGLE_SATURATION_THRESHOLD
    ...
    def publish_logs(self, CS, start_time, actuators, v_acc, a_acc, lac_log):
        ...
        CC.cruiseControl.accelOverride = self.CI.calc_accel_override(CS.aEgo, self.sm['longitudinalPlan'].aTarget,
                                                                     CS.vEgo, self.sm['longitudinalPlan'].vTarget)
