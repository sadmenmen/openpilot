...
class DriverStatus():
    ...
    def set_policy(self, model_data):
        ...
        self.pose.cfactor = interp(ep, [0, 0.5, 1], [_METRIC_THRESHOLD_STRICT, _METRIC_THRESHOLD,
                                                     _METRIC_THRESHOLD_SLACK]) / _METRIC_THRESHOLD
        self.blink.cfactor = interp(ep, [0, 0.5, 1], [_BLINK_THRESHOLD_STRICT, _BLINK_THRESHOLD,
                                                      _BLINK_THRESHOLD_SLACK]) / _BLINK_THRESHOLD
    ...
    def get_pose(self, driver_state, cal_rpy, car_speed, op_engaged):
        ...
        self.pose.roll, self.pose.pitch, self.pose.yaw = face_orientation_from_net(driver_state.faceOrientation,
                                                                                   driver_state.facePosition, cal_rpy,
                                                                                   self.is_rhd_region)
    ...
    def update(self, events, driver_engaged, ctrl_active, standstill):
        ...
        if (not (self.face_detected and self.hi_stds * DT_DMON <= _HI_STD_FALLBACK_TIME) or (
                self.driver_distraction_filter.x > 0.63 and self.driver_distracted and self.face_detected)) and \
