...
class CarController():
    ...
    def rough_speed(self, lead_distance):
        if self.prev_lead_distance != lead_distance:
            ...
            self.rough_lead_speed = (self.lead_distance_counter * self.rough_lead_speed) / (
    ...
    def update(self, enabled, CS, frame, actuators,
        ...
        brake, self.braking, self.brake_steady = actuator_hystereses(actuators.brake, self.braking, self.brake_steady,
        ...
        if (frame % 10) == 0:
            idx = (frame // 10) % 4
            can_sends.extend(
                hondacan.create_ui_commands(self.packer, pcm_speed, hud, CS.CP.carFingerprint, CS.is_metric, idx,
                                            CS.CP.openpilotLongitudinalControl, CS.stock_hud))
        ...
        if not CS.CP.openpilotLongitudinalControl:
            ...
            if pcm_cancel_cmd:
                can_sends.append(
                    hondacan.spam_buttons_command(self.packer, CruiseButtons.CANCEL, idx, CS.CP.carFingerprint))
                ...
                if CS.CP.carFingerprint in (CAR.ACCORD, CAR.ACCORD_15, CAR.ACCORDH):
                    ...
                    if CS.lead_distance > (self.stopped_lead_distance + 15.0) or rough_lead_speed > 0.1:
                        self.stopped_lead_distance = 0.0
                        can_sends.append(
                            hondacan.spam_buttons_command(self.packer, CruiseButtons.RES_ACCEL, idx,
            ...
            if (frame % 2) == 0:
                ...
                if CS.CP.carFingerprint in HONDA_BOSCH:
                    ...
                    pump_on, self.last_pump_ts = brake_pump_hysteresis(apply_brake, self.apply_brake_last,
